...
from base_bilstm.model import get_model_inputs, data_to_inputs_and_gold
from frozen_glove.glove_embedding import load_glove, GloVeEmbeddingLayer
from frozen_glove.model import Model
...
class GloVeBilstmClassifier(Classifier):
    def __init__(self, size_lstm, size_feed_forward,
        ...
        self.save_config = {'size_lstm': size_lstm,
        ...
        word_vocab, glove_embeddings = load_glove(vocab_only=True)
        ...
        self.model = Model(len(self.TE_label_vocab), size_lstm,
    ...
    def data_to_inputs_and_gold(self, gold_data, silver_data, dev_data, labeled):
        ...
        if silver_data:
            silver_inputs_labels = data_to_inputs_and_gold(gold_data, labeled, self.word_vocab, self.TE_label_vocab,
                                                           ...
                                                           self.max_words_per_node, self.disable_handcrafted_features)
    ...
    def load(cls, model_file):
        with open(model_file + '.config', 'r', encoding='utf-8') as config_file:
            ...
            classifier = cls(config['size_lstm'], config['size_feed_forward'],
